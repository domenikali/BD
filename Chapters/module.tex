...
Before starting the implementation is key to define how the simulator works and the constraints of the architecture and the software running the simulator.

\section{GVSoC Integration}\label{sec:gvsoc_integration}
...
\subsection{Module Structure}\label{sec:module_struct}
The implementation consists of three components all connected through the Simulation Engine, as shown in Figure \picref{fig:pcm_gvsoc_integration}.

\textbf{C++ Model Class:} The core logic is implemented in a class inheriting from \textit{vp::component}. This class handles incoming memory requests, maintains the PCM crossbar state, and executes MVM operations.

\textbf{Python Generator:} A configuration script generates the module's JSON configuration file and defines its connections to other system components. The generator specifies the module's address range, memory size, and available operational modes.

\textbf{JSON Configuration:} The generated JSON file contains all module parameters including matrix dimensions, cell precision, and timing characteristics.

\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{Figures/GVSoC_Struct.png}
    \caption{GVSoC Simulator Structure}
    \label{fig:pcm_gvsoc_integration}
\end{figure}

\subsection{Bus Interface}\label{sec:bus_inter}
The module connects to the system through GVSoC's wire mechanism. A request handler with signature \textit{vp::IoReqStatus handler(vp::Block* block, vp::IoReq* req)} processes incoming read and write operations from the simulated RISC-V cores.

The memory map exposes three regions:
\begin{itemize}
  \item \textbf{Weight Memory:} Direct access to PCM cell values for reading and writing weights values.
  \item \textbf{Input/Output Vectors:} Buffers for MVM operands and results.
  \item \textbf{Control Registers:} Configuration of operation mode, matrix dimensions, and computation triggers
\end{itemize}

\subsection{Timing Model}\label{sec:timing_mod}
GVSoC is an event-driven simulator, the timing is represented in two main ways as shown in figure \picref{fig:Event_engine}:
\begin{itemize}
  \item \textbf{Functional Timing:} Each operations (e.g., read, write) can be performed in zero time, meaning that the operation is completed as soon as it is called but a latency is added to the event to guarantee correct timing simulation.
  \item \textbf{Event Scheduling:} Operations can be scheduled to occur after a specific delay by enqueuing the event in the simulator's event queue. Once the delay elapses, the event triggers the module's \textit{ClockEvent} handler to process the operation.
\end{itemize}
The module will make good use of these mechanisms, simulating the timing characteristics of PCM operations.

\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{Figures/Event_engine.png}
    \caption{GVSoC Event Engine}
    \label{fig:Event_engine}
\end{figure}
\subsection{Power Management}\label{sec:power_man}
...
A second, more optimised approach would be to use a "flat" n-dimensional array. This solution requires more advanced indexing calculations, leading to significantly fewer instructions for memory allocation and deallocation. Implementing this approach results in noticeable performance improvements. 
Analysis of a test program using Cachegrind (a Valgrind tool that simulates cache access) shows a slight decrease in cache misses and a marked reduction in memory reads. The decrease in cache misses is due to the sequential storage of data, which is read from memory in a similar sequential manner. The reduction in memory reads occurs because contiguous memory allocation eliminates the overhead of pointer dereferencing.
...
To implement this optimisation, a temporary variable is used to store the result of each row computation before writing it back to the output vector.
...
Table \ref{tab:asm_opt_comparison} provides the assembly code generated for each of the cases, analysing it is shows why shuch optimisation is beneficial only when compiler optimisation are enabled.
In the non-optimised case the compiler store that value on the stack and retrieves it at each iteration degrading the overall performances.\\
...
\section{Memory Mapping and Configuration}\label{sec:gvsoc_int}

...
The main metric used to evaluate the performance of the algorithms is the time to complete a single MVM computed with the aid of the $std::high\_resolution\_clock$.
\clearpage
\begin{figure}[!htb]
\begin{adjustwidth}{-1.75cm}{-1.75cm} 

\begin{minipage}{1.25\textwidth}
    {\includegraphics[width=\textwidth]{Figures/disper_opt.png}
    \label{fig:no_opt_plot}}
\end{minipage}
\hfill
\begin{minipage}{1.25\textwidth}
    {\includegraphics[width=\textwidth]{Figures/disper.png}
    \label{fig:opt_plot}}
\end{minipage}
\caption{Scatter plots of the results with and without compiler optimisation. \label{fig:plots}}
\end{adjustwidth}
...
Starting with the instruction counts (Ir), we observe a $4\times$ reduction in instructions executed when compiled with O3 and up to $6\times$ without compiler optimisations. This outcome aligns with the overall memory access reductions seen in both data reads (Dr) and writes (Dw).
...
However, a closer look at the miss count reveals that the remaining misses are the result of compulsory misses that Cachegrind cannot simulate away by including hardware prefetching simulation.
...
Under O0 compilation, data reads decrease by a factor of $4\times$ compared to the naive implementation, while under O3 this improvement reaches $40\times$.
More significantly, write operations are reduced by $250\times$ (O0) and $260\times$ (O3).
...
Cachegrind results indicate that the multithreaded versions consume approximately the same amount of memory accesses as the single-threaded optimised version.
...
Then tests with randomised vector and matrix values with all ones weights were executed, the result of the MVM is known and can be easily checked as $Y_i=\sum_{j}^{N}1\cdot X_j$, making the expected result appear on every output value.

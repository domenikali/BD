...
\begin{itemize}
  \item \textbf{Functional Timing:} Each operations (e.g., read, write) can be performed in zero time, meaning that the operation is completed as soon as it is called but a latency is added to the envent to guarantee correct timing simulation.
  \item \textbf{Event Scheduling:} Operations can be scheduled to occur after a specific delay by enqueing the event in the simulator's event queue. Once the delay elapses, the event triggers the module's \textit{ClockEvent} handler to process the operation.
\end{itemize}
